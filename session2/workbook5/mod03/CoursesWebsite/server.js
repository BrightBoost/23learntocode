...
app.get('/api/instructors', function (req, res) {
    console.log("Got a GET request for instructors");
    let data = fs.readFileSync(__dirname + "/data/" + "instructors.json", 'utf8');
    data = JSON.parse(data);
    console.log("Returned data is: ");
    console.log(data);
    res.end(JSON.stringify(data));
...
app.get('/api/instructors/:id', function (req, res) {
    let id = req.params.id;
    console.log("Got a GET request for instructor " + id);
    let data = fs.readFileSync(__dirname + "/data/" + "instructors.json", 'utf8');
    data = JSON.parse(data);
    console.log("In callback after read");
    let match = data.find(instructor => instructor.instructorId == id);
    if (match) {
        console.log("Returned data is: ");
        console.log(match); 
        res.end(JSON.stringify(match));
