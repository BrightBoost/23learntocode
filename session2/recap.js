...
posts.filter(post => post.userId == 1).forEach(post => console.log(post.title));

posts.forEach(post => {
    let userObj = users.find(user => user.id == post.userId);
    console.log(post.title + " by " + userObj.username);
})